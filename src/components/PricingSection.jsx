...
export default function PricingSection() {
  ...
  return (
    <section id="pricing" className="relative bg-cream px-6 py-20 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="mx-auto max-w-xl text-center">
          <span className="text-sm font-semibold uppercase tracking-wide text-slate">Pricing</span>
        ...
        <div className="mt-9 flex items-center justify-center gap-4">
          ...
          <button
            ...
            className="relative h-7 w-14 rounded-full bg-ink/10"
          ...
          <span className={`text-sm font-semibold ${yearly ? "text-ink" : "text-slate/50"}`}>
            Yearly{" "}
            <span className="rounded-full bg-chip-green px-2 py-0.5 text-xs text-ink">save 20%</span>
          </span>
        </div>
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {PLANS.map((plan) => (
            <div
              key={plan.name}
              className={`relative flex flex-col rounded-2xl p-7 transition hover:-translate-y-0.5 ${
                plan.highlight ? "bg-ink text-white md:scale-105" : "border border-border bg-white text-ink"
              }`}
            >
              {plan.highlight && (
                <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 rounded-full bg-white px-4 py-1 text-xs font-bold text-ink">
                  Most popular
                </span>
              )}
              <h3 className="font-heading text-3xl font-bold">{plan.name}</h3>
              <p className={`mt-1 text-sm ${plan.highlight ? "text-white/70" : "text-slate"}`}>{plan.tagline}</p>

              <div className="mt-6 flex items-end gap-1">
                <span className="text-4xl font-extrabold">${yearly ? plan.yearly : plan.monthly}</span>
                <span className={`pb-1 text-sm ${plan.highlight ? "text-white/70" : "text-slate/70"}`}>/mo</span>
              </div>
              ...
              <a
                ...
                className={`mt-8 rounded-full px-5 py-3 text-center text-sm font-bold transition hover:-translate-y-0.5 ${
                  plan.highlight ? "bg-white text-ink" : "bg-ink text-white hover:bg-ink/85"
                }`}
              >
                Get started
              </a>
